...
class Ghost_Framework_Fonts {
    ...
    public function enqueue_all_fonts_assets() {
        ...
        if ( is_admin() || ! empty( $fonts ) ) {
            ...
            wp_enqueue_script( 'ghost-framework-fonts-loader', Ghost_Framework::get_url() . '/assets/js/fonts-loader.min.js', array( 'webfontloader' ), '1.3.3', false );
    ...
    public function add_default_site_fonts( $fonts ) {
        $fonts['default'] = array(
            'name'  => esc_html__( 'Default Fonts Site', 'skylith' ),
            'fonts' => array(
                array(
