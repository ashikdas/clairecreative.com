...
class Ghost_Framework {
    ...
    public static function admin_enqueue_scripts() {
        wp_enqueue_style( 'ghost_admin', self::$framework_url . '/assets/css/admin.css', array(), '1.3.3' );
    }
    ...
    public static function maybe_add_kirki_options() {
        if ( ! empty( self::$registered_options ) ) {
            foreach ( self::$registered_options as $option ) {
                // add field.
                Ghost_Framework_Kirki::add_field( 'skylith', $option );
            }
        }
    }
    ...
    public static function add_config( $args ) {
        Ghost_Framework_Kirki::add_config( 'skylith', $args );
    }
    ...
    public static function get_option( $name ) {
        $value = Ghost_Framework_Kirki::get_option( 'skylith', $name );
        ...
        if ( is_home() || is_front_page() ) {
            ...
            if ( function_exists( 'is_bbpress' ) && is_bbpress() ) { // supported bbpres breadcrumbs.
                ...
                if ( get_query_var( 'paged' ) ) {
                    $sub_result .= ' (';
                    // @codingStandardsIgnoreLine
                    $sub_result .= esc_html__( 'Page', 'skylith' ) . ' ' . get_query_var( 'paged' );
                    $sub_result .= ')';
                }
                if ( $args['home'] ) {
                ...
                if ( is_category() ) {
                    ...
                    $result .= $args['before'] . esc_html__( 'Day', 'skylith' ) . '&nbsp;' . get_the_time( 'd' ) . $sub_result . $args['after'];
                } elseif ( is_month() ) {
                    ...
                    $result .= $args['before'] . esc_html__( 'Year', 'skylith' ) . '&nbsp;' . get_the_time( 'Y' ) . $sub_result . $args['after'];
